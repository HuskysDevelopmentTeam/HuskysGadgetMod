...
public class BlockServerRack extends BlockFacing {
    ...
    public BlockServerRack() {
        super(Material.ANVIL, Reference.MOD_ID, "server_rack");
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if (tileEntity instanceof TileEntityServerRack) {
            TileEntityServerRack server = (TileEntityServerRack) tileEntity;

            if (playerIn.isSneaking()) {
                if (!worldIn.isRemote) {
                    server.connectUnconnectPower();
                }
            }
        }
        return true;
