...
import net.thegaminghuskymc.gadgetmod.api.app.Layout.Background;
...
public abstract class Dialog extends Wrappable
{
    ...
    public static class Confirmation extends Dialog {
        ...
        public void init()
        {
            ...
            buttonPositive.setSize(30, 15);
            buttonNegative.setClickListener((ClickListener) (mouseX, mouseY, mouseButton) -> {
            ...
            for(int y = -range; y < range + 1; y++)
            {
                for(int z = -range; z < range + 1; z++)
                {
                    for(int x = -range; x < range + 1; x++)
                    {
                        ...
                        if(state.getBlock() == GadgetBlocks.PRINTER)
                        {
